...
# from dataloader2 import video_dataset1
from dataloader_Li import realDataLoader
...
def main():
    ...
    parser.add_argument('--imgdir', type=str, default="/home/moriyama/real_images/", help='path of real_images')
    parser.add_argument('--imgdir2', type=str, default="/home/moriyama/ani_images/", help='path of ani_images')
    # parser.add_argument('--videodir', type=str, default="~//", help='path of videos')
    ...
    # videoデータセットを読み込み
    real_video_dataset = realDataLoader(folderPath=opt.imgdir)
    ani_video_dataset = realDataLoader(folderPath=opt.imgdir2)
    ...
    for epoch in range(opt.epochs):
        for i, (real_video, ani_video) in enumerate(zip(real_video_loader, ani_video_loader)):
            ...
            #perceptual loss
            per_loss = criterion(Vgg(fake_ani),Vgg(real_video))
            ...
            if i % 40 == 0:
                ...
                print("Epoch:{:.0f},Batch/Batchs: {:.0f}/{:.0f}, D_loss: {:.3f} d_loss_realani: {:.3f}  d_loss_fakeani: {:.3f} "
                      "G_loss: {:.3f} g_loss: {:.3f} per_loss: {:.3f}".format(
                        epoch+1,i,len(ani_video_loader),
