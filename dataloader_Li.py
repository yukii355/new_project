...
class realDataLoader(data.DataLoader):
    ...
    def __getitem__(self, index):
        img_path_list = self.trainList[index] # img_path_listには12枚の画像
        h, w, c = cv2.imread(img_path_list[0]).shape # h=600,w=800,c=3
        ...
        for image_path in img_path_list:
            tmp = cv2.resize(cv2.imread(image_path), (scaleX, scaleY))[:, :, (2, 1, 0)] # tmp = (480,360,3)
